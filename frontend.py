...
import base64
from backend import ShortestPath  # Ensure backend.py is in the same directory or adjust the import path
import googlemaps
    ...
    if st.button("Submit"):
        if start_location and destination_location:
            ...
            if start_coords and destination_coords:
                # Initialize backend ShortestPath instance
                ...
                if map_url:
                    st.write(f"[Google Maps Link]({map_url})")
                else:
